...
def quantity(journalists)
  qty = journalists.count
  puts "Combien y a t'il de journalistes dans cet array ? "
  puts qty
end

def numbered(journalists)
  move = 0
  num = 0
  while journalists[move]
    if journalists[move] =~ /[0-9]/
      move += 1
      num += 1
    else
      move += 1
...
def audigy(journalists)
  move = 0
  aud = 0
  while journalists[move]
    if journalists[move].downcase.include?("aude")==true
      move += 1
      aud += 1
    else
      move += 1
...
def upcase_front(journalists)
  move = 0
  upf = 1
  nbr = 0
  while journalists[move]
    if journalists[move][upf] =~ /[A-Z]/
      nbr += 1
    end
    move += 1
  end
...
def upcase_any(journalists)
  move = 0
  nmr = 0
  while journalists[move]
    if journalists[move] =~ /[A-Z]/
      move += 1
      nmr += 1
    else
      move +=1
...
def underscoring(journalists)
  move = 0
  nmr = 0
  while journalists[move]
    if journalists[move] =~ /_/
      move += 1
      nmr += 1
    else
      move +=1
...
sorting(journalists)
quantity(journalists)
numbered(journalists)
audigy(journalists)
upcase_front(journalists)
upcase_any(journalists)
underscoring(journalists)
puts "Winston777"
